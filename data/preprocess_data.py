...
def extract_features(single_game_data, index, previous_move_location=-1):
    ...
    return all_features, label, is_winning_move
...
def add_rotated_games(data):
    ...
    for i, game in enumerate(data):
        if i % 500 == 0:
            print("%d game flipped of %d" % (i, len(data)))
...
def preprocess():
    ...
    for i, game in enumerate(data):
        ...
        for game_step in range(len(game["states"])):
            features, previous_move, does_win_or_draw = extract_features(game, game_step, previous_move)
            if does_win_or_draw:
